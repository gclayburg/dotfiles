...
if [[ -L ${BACKUP_HOME}/current ]]; then
  ...
  do_shell rsync -HaxP --stats --link-dest=${BACKUP_HOME}/current ${SOURCE_DIR} ${BACKUP_HOME}/back-$date
  ...
  if [[ -w ${BACKUP_HOME} ]]; then
    do_shell rsync -HaxP --stats ${SOURCE_DIR} ${BACKUP_HOME}/back-$date
    do_shell ln -s back-$date ${BACKUP_HOME}/current
  else
